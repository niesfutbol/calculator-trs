...
league <- read_csv("results/league_with_key_passes_140_2022.csv")
...
league_to_fit <- cleaned_league |>
  ...
  select(
    c("goal", "shots_insidebox_kpnb", "shots_outsidebox_kpnb", "shots_insidebox_nkpnb", "shots_outsidebox_nkpnb")
  )
...
xG <- xGoal_all_league[["88_2021_kp"]]
league <- cleaned_league %>%
  mutate(xgoal_kp = calculate_xgoal_kp(
    xG,
    shots_insidebox_kpnb,
    shots_outsidebox_nkpnb,
    shots_insidebox_nkpnb,
    shots_outsidebox_kpnb,
    total_penalties
  ))
