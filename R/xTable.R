...
calcualte_attack_strength_for_each_team <- function(ids, league, mode = "mean") {
  comprehenr::to_vec(for (id in ids) GET_STRENGTH_ATTACK[[mode]](league, id))
}

calcualte_defense_strength_for_each_team <- function(ids, league, mode = "mean") {
...
concatenate_strength_attack_defense <- function(names, league, mode = "mean") {
  ids <- names[["ids"]]
  attack <- calcualte_attack_strength_for_each_team(ids, league, mode)
  defense <- calcualte_defense_strength_for_each_team(ids, league, mode)
